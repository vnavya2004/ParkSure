...
void _signInWithEmailAndPassword() async {
      ...
      if (user.email == "gfg882004@gmail.com") {
        ...
        // Check user role from Firestore
         FirebaseFirestore.instance
          ...
          } else   if (doc.exists && doc['role'] == 'admin') {
            // Navigate to Owner Page
            Navigator.of(context).pushReplacement(
              MaterialPageRoute(builder: (_) => AdminPage()),
            );
  ...
  // Check if the user has selected a Google account
  if (googleSignInAccount == null) {
    // If no account was selected, show a toast and return to same page
    Fluttertoast.showToast(
      msg: "No account selected. Please try again.",
      toastLength: Toast.LENGTH_SHORT,
      gravity: ToastGravity.BOTTOM,
    );
    return 'Sign-in failed';  // Do nothing and return early if no account was selected
  }

  // If a Google account is selected, proceed with authentication
  ...
  try {
    ...
    if (user != null) {
      ...
      // After successful sign-in, check user role and navigate accordingly
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (context) {
            // Navigate based on user role
            return Dashboard(user: user);  // Make sure to handle null user
          },
        ),
      );
