    ...
    Container(
      ...
      child: ElevatedButton(
        ...
        onPressed: dropdownValue == null
                                  ? null // Disable button if no vehicle is selected
                                  : () async {
                                      ...
                                      if (checkInDateTime.isBefore(DateTime.now())) {
                                        Fluttertoast.showToast(
                                            msg: "Check-in time must be in the future");
                                        return;
                                      }
...
Future<void> sendEmail() async {
  ...
  final Map<String, dynamic> emailData = {
    ...
    "content": [
      {
        "type": "text/html", // Send as HTML email
        "value": """
          <html>
            <body>
              <h1>Booking Confirmation</h1>
              <p>Your parking spot at <strong>${widget.spot['name']}</strong> has been successfully booked. Below are the details:</p>
              <ul>
               
                <li><strong>Vehicle Number:</strong> ${dropdownValue}</li>
                <li><strong>Check-in Time:</strong> ${_checkInTime.hour.toString().padLeft(2, '0')}:${_checkInTime.minute.toString().padLeft(2, '0')} on ${DateFormat('dd MMM yyyy').format(_checkInDate!)} </li>
                <li><strong>Check-out Time:</strong> ${_checkOutTime.hour.toString().padLeft(2, '0')}:${_checkOutTime.minute.toString().padLeft(2, '0')} on ${DateFormat('dd MMM yyyy').format(_checkOutDate!)} </li>
                <li><strong>Total Cost:</strong> \u{20B9} ${(DateTime(
                  _checkOutDate!.year,
                  _checkOutDate!.month,
                  _checkOutDate!.day,
                  _checkOutTime.hour,
                  ...
                  _checkInTime.hour,
                  _checkInTime.minute,
                )).inMinutes / 60 * widget.spot['costPerHour']).ceil()}</li>
              </ul>
              <p>Thanks for using ParkSure! We look forward to serving you.</p>
            </body>
          </html>
        """
      }
